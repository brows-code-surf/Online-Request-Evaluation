...
class PurchaseRequest {
    ...
    static async createPurchaseRequest(headerData, detailsData, creatorName) {
        ...
        try {
            ...
            // Use the reference number from headerData (generated by the form)
            const referenceNo = headerData.referenceNo || `PR-${Date.now()}`;

            // Extract numeric part from reference number (remove prefix)
            // e.g., "SCPRO-123" becomes "123"
            let referenceNum = '';
            if (referenceNo) {
                const parts = referenceNo.split('-');
                if (parts.length > 1) {
                    referenceNum = parts[parts.length - 1]; // Get the last part after the last dash
                } else {
                    referenceNum = referenceNo; // Fallback if no dash
                }
            }
            ...
            await connection.request()
                ...
                .input('referenceNo', headerData.referenceNo)
